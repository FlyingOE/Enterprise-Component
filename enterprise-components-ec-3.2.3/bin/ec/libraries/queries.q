...
.example.tradeStats:{[] .hnd.h[`stocks.rdb] "select cnt:count i by time.hh from quote"};

/F/rdb quote and trade tables records joined based on time and sym columns
/E/.example.tradeAndQuote[]
.example.tradeAndQuote:{[] .hnd.h[`stocks.rdb] "aj[`sym`time;trade;quote]"};

/F/last price by symbol from rdb trade table
/E/.example.currentPrices[]
.example.currentPrices:{[] .hnd.h[`stocks.rdb] "select last price by sym from trade"};

/F/volume-weighted average price by symbol from rdb
/E/.example.vwap[]
.example.vwap:{[] .hnd.h[`stocks.rdb] "select vwap: size wavg price by sym from trade"};

/F/volume-weighted average price by symbol from rdb, functional form
/E/.example.functionalVwap[]
.example.functionalVwap:{[] .hnd.h[`stocks.rdb](?;`trade;();(enlist `sym)!enlist `sym;(enlist `vwap)!enlist (wavg;`size;`price))};

/F/volume-weighted average price by symbol and date from hdb
/E/.example.dailyVwap[]
.example.dailyVwap:{[] .hnd.h[`stocks.hdb] "select vwap: size wavg price by sym,date from trade"};

/F/last price in 10 minutes bars for first symbol in rdb trade table 
/E/.example.lastPrice10MinutesBar[]
.example.lastPrice10MinutesBar:{[] .hnd.h[`stocks.rdb] "select last price by 15 xbar time.minute from trade where sym = first sym"};
...
.example.functionalLastPriceNMinutesBar:{[n;sym] .hnd.h[`stocks.rdb](?;`trade;enlist (=;`sym; enlist sym);(`sym`minute)!(`sym;(xbar;n;`time.minute));(enlist `price)!enlist (last;`price))};

/F/orders from rdb quote table for which bid size was greater than average by symbol 
/E/.example.bigBuyOrders[]
.example.bigBuyOrders:{[] .hnd.h[`stocks.rdb]"select from quote where bidSize > (avg;bidSize) fby sym"};;
...
/E/syms: exec sym from .hnd.h[`stocks.hdb]("2#select from trade");    
...
.example.ohlcVwap:{[sym;d;sTime;eTime;binSize]
 aggrs:`open`high`low`close`size`vwap!((first;`price);(max;`price);(min;`price);(last;`price);(sum;`size);(%;(wsum;`size;`price);(sum;`size)));
  ...
  .hnd.h[`stocks.hdb] (?;`trade;wheres;bys;aggrs)
  };
...
.example.ohlcVwap2:{[sym;d;sTime;eTime;binSize]
  ...
  .query.data[`stocks.rdb`stocks.hdb;`trade;wheres;d;bys;aggrs]
  };


/ Calculate vwap by sym every 10 minutes
.example.vwapTrade:{.hnd.h[`stocks.rdb]"`time xasc select vwap:size wavg price by sym, time:10 xbar time.minute from trade"};
